...
function App() {
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    if (nextIndex !== selectedIndex) {
      ...
      // Improved scrolling: find the tile within the container
      setTimeout(() => {
  ...
  }, [selectedIndex, isFocusMode, sessions.length, sessions]);
        ...
        {sessions.length === 0 ? (
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 xl:grid-cols-12 gap-2">
            {sessions.map((session, index) => (
              <div key={session.id} data-index={index}>
                <SessionTile session={session} isSelected={index === selectedIndex} onClick={() => setSelectedIndex(index)} />
              </div>
            ))}
          </div>
        )}
